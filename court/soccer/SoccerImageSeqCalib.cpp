...
#include "SoccerImageSeqCalib.h"
#include "wwosSoccerCourt.h"
#include "WWoSSoccerCourtUtil.h"

#include <vnl/vnl_matlab_filewrite.h>

/********************************        WWoSSoccerReadWriteCamera     *****************************/


bool WWoSSoccerReadWriteCamera::read_cameras_from_file(const char *file_name, ImageSequenceCameras & cameraSequence)
{
    assert(file_name);
...
bool WWoSSoccerReadWriteCamera::write_cameras(const char *file, const ImageSequenceCameras & cameras)
{
    assert(file);
...
bool WWoSSoccerReadWriteCamera::read_ptz_from_file(const char *file_name, ImageSequenceCameras & sequence)
{
    FILE *pf = fopen(file_name, "r");
    if (!pf) {
        printf("can not open %s\n", file_name);
    ...
    while (1) {
        ...
        // PTZToCameraOldNotUsed PTZToCamera
        vpgl_perspective_camera<double> camera;
        bool isCamera =  WWoSSoccerCourtUtil::PTZToCamera(pan, tilt, fl, camera);
        if (isCamera) {
            FrameData data;
            data.fn_ = fn;
            data.camera_ = camera;
...
bool WWoSSoccerReadWriteCamera::write_ptzs(const char *file, const ImageSequenceCameras & cameras)
{
    assert(file);
    FILE *pf = fopen(file, "w");
    if (!pf) {
    ...
    vnl_matrix<double> ptzData((int)cameras.frames_.size(), 4, 0);
    for (int i = 0; i<cameras.frames_.size(); i++) {
        ...
        double tilt = 0;
        double ssd = 10000;
        bool isPTZ = WWoSSoccerCourtUtil::cameraToPTZ(cameras.frames_[i].camera_, pan, tilt, fl);
        if (!isPTZ) {
            printf("Warning: can not convert to ptz. write 0 0 0 as focal lenth, pan, tilt \n");
           // continue;
        }
    ...
    // save to .mat file
    vcl_string matFile = vcl_string(file);
    matFile[matFile.size()-3] = 'm';
    matFile[matFile.size()-2] = 'a';
    matFile[matFile.size()-1] = 't';
    
    vnl_matlab_filewrite writer(matFile.c_str());
    writer.write(ptzData, "fn_ptz");
    printf("save to %s\n", matFile.c_str());
