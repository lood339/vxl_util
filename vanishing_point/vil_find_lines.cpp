#include <list>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>

#define ELLIPSE_THRESH 32

#include "vil_find_lines.h"
#include "vxl_canny_edgelet.h"
#include "vil_plus.h"

//neighborhoods
const int xinc[8] = {-1,-1,-1, 0,0, 1,1,1};
const int yinc[8] = {-1, 0, 1,-1,1,-1,0,1};

template <class T> const T& max (const T& a, const T& b) {
    return !(b>a)?a:b;
}

struct line_param
{
    float a;
    float b;
    float c;
    float w;
    
    line_param(float _a,float _b, float _c, float _w):a(_a),b(_b),c(_c),w(_w){}
};

struct coords
{
    int x;
    int y;
    int d;
    
    coords(int _x,int _y, int _d):x(_x),y(_y),d(_d){}
};

static std::list<coords> floodFill(int *data, int cx, int cy, int img_height, int * rads, int rads_size, int * v_steps, int * dist);
static int fit_ellipse(std::list<coords> & results, float * vec_data);
static int eigens(float * cov_data, float * vec_data);

/*
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    
    //init matrix
    int * data = (int*)mxGetData(prhs[0]);
    
    int width = int(mxGetN(prhs[0]));
    int height = int(mxGetM(prhs[0]));
    
    //rings radius
    int * rads = (int*)mxGetData(prhs[1]);
    int rads_size = int(mxGetN(prhs[1]));
    int patch_size = rads[rads_size-1]*2 + 1;
    
    //v_step for faster indexing
    int * v_steps = (int*)malloc(sizeof(int)*patch_size);
    for(int i = 0; i < patch_size; i++)
        v_steps[i] = i*patch_size;
    
    //mask with distances
    int * dist = (int*)malloc(sizeof(int)*patch_size*patch_size);
    for(int i=0; i<patch_size; i++)
        for(int j=0; j<patch_size; j++)
            dist[v_steps[i] + j] = max(abs(i-rads[rads_size-1]), abs(j-rads[rads_size-1]));
    
    
    std::list<line_param> lines;
    
    //normalize values
    float w_c = (width - 1)/2.f;
    float h_c = (height - 1)/2.f;
    float norm = (max(w_c, h_c) - rads[rads_size-1]);
    
    float vec_data[4];
    //for each edge point in image with padds
    for(int i=rads[rads_size-1]; i <  width - rads[rads_size-1]; i++)
        for(int j=rads[rads_size-1]; j <  height - rads[rads_size-1]; j++)
        {
            if (data[i * height + j] == 1)
            {
                //call flood fill with array of rings radius
                std::list<coords> results = floodFill(data, i, j, height, rads, rads_size, v_steps, dist);
                
                //fit ellipse
                if(fit_ellipse(results, vec_data) == 1)
                {
                    //push if ok
                    float c = -vec_data[0]*((j - h_c)/norm) - vec_data[1]*((i - w_c)/norm);
                    lines.push_back(line_param(vec_data[1], vec_data[0], c, 1.f));
                }
            }
        }
    
    int lines_size[2] = {4, lines.size()};
    mxArray * mx_lines = mxCreateNumericArray(2, lines_size, mxSINGLE_CLASS, mxREAL);
    float * mx_lines_data = (float*)mxGetData(mx_lines);
    
    int c = 0;
    for (std::list<line_param>::iterator it = lines.begin(); it!=lines.end(); ++it, c++)
    {
        mx_lines_data[c*4] = it->a;
        mx_lines_data[c*4 + 1] = it->b;
        mx_lines_data[c*4 + 2] = it->c;
        mx_lines_data[c*4 + 3] = it->w;
    }
    
    plhs[0] = mx_lines;
    //free(labData);
    //plhs[0] = idx;
    
    free(v_steps);
    free(dist);
}
*/

void VilFindLines::findLines(const vil_image_view<vxl_byte> & image,
                                         const PCLineParameters & para,
                                         vcl_vector<vnl_vector_fixed<double, 4> > & outLines)
{
    //init matrix
    CannyParameter cannyPara;
    cannyPara.min_gradient_magnitude2_ = para.min_canny_gradient_magnitude2_;
    vil_image_view<vxl_byte> edgeMask;
    VxlCannyEdgelet::canny(image, cannyPara, edgeMask);
    
    // pad edge image
    const int org_width  = image.ni();
    const int org_height = image.nj();
    vil_image_view<vxl_byte> expanedEdgeImg(org_width + 2 * para.patchSize, org_height + 2 * para.patchSize, 1);
    expanedEdgeImg.fill(0);
    for (int j = 0; j<edgeMask.nj(); j++) {
        for (int i = 0; i<edgeMask.ni(); i++){
            expanedEdgeImg(i + para.patchSize, j + para.patchSize) = edgeMask(i, j);
        }
    }
    
 //   VilPlus::vil_save(expanedEdgeImg, "edge.jpg");
    
    const int width  = expanedEdgeImg.ni();
    const int height = expanedEdgeImg.nj();
    
    //rings radius
    int rads[] = {para.patchSize, para.patchSize};
    int rads_size = 2;
    int patch_size = rads[rads_size-1]*2 + 1; // ? fixed size?
    
    int *data = new int[expanedEdgeImg.nj() * expanedEdgeImg.ni()];
    for (int j = 0; j<expanedEdgeImg.nj(); j++) {
        for (int i = 0; i<expanedEdgeImg.ni(); i++) {
            if (expanedEdgeImg(i, j) == 255) {   // column dominant as in matlab
                data[i * height + j] = 1;
            }
            else{
                data[i * height + j] = 0;
            }
        }
    }    
    
    //v_step for faster indexing
    int * v_steps = (int*)malloc(sizeof(int)*patch_size);
    for(int i = 0; i < patch_size; i++){
        v_steps[i] = i*patch_size;
    }
    
    //mask with distances
    int * dist = (int*)malloc(sizeof(int)*patch_size*patch_size);
    for(int i=0; i<patch_size; i++){
        for(int j=0; j<patch_size; j++){
            dist[v_steps[i] + j] = max(abs(i-rads[rads_size-1]), abs(j-rads[rads_size-1]));
        }
    }
    
    
    std::list<line_param> lines;
    
    //normalize values
    float w_c = (width - 1)/2.f;
    float h_c = (height - 1)/2.f;
    float norm = (max(w_c, h_c) - rads[rads_size-1]);
    
    float vec_data[4];
    //for each edge point in image with padds
    for(int i=rads[rads_size-1]; i <  width - rads[rads_size-1]; i++){
        for(int j=rads[rads_size-1]; j <  height - rads[rads_size-1]; j++)
        {
            if (data[i * height + j] == 1)
            {
                //call flood fill with array of rings radius
                std::list<coords> results = floodFill(data, i, j, height, rads, rads_size, v_steps, dist);
                
                //fit ellipse
                if(fit_ellipse(results, vec_data) == 1)
                {
                    //push if ok
                    float c = -vec_data[0]*((j - h_c)/norm) - vec_data[1]*((i - w_c)/norm);
                    lines.push_back(line_param(vec_data[1], vec_data[0], c, 1.f));
                }
            }
        }
    }
    
    // output lines
    for (std::list<line_param>::iterator ite = lines.begin(); ite != lines.end(); ite++) {
        vnl_vector_fixed<double, 4> line;
        line[0] = ite->a;
        line[1] = ite->b;
        line[2] = ite->c;
        line[3] = ite->w;
        outLines.push_back(line);
    }
    
    delete []data;
    free(v_steps);
    free(dist);
    
    printf("find %lu lines\n", outLines.size());
}

void VilFindLines::findLinesFromEdges(const vil_image_view<vxl_byte> & edges, const PCLineParameters &  para,
                                      vcl_vector<vnl_vector_fixed<double, 4> > & outLines)
{
    // pad edge image
    const int org_width  = edges.ni();
    const int org_height = edges.nj();
    vil_image_view<vxl_byte> expanedEdgeImg(org_width + 2 * para.patchSize, org_height + 2 * para.patchSize, 1);
    expanedEdgeImg.fill(0);
    for (int j = 0; j<edges.nj(); j++) {
        for (int i = 0; i<edges.ni(); i++){
            expanedEdgeImg(i + para.patchSize, j + para.patchSize) = edges(i, j);
        }
    }
    
    //   VilPlus::vil_save(expanedEdgeImg, "edge.jpg");
    
    const int width  = expanedEdgeImg.ni();
    const int height = expanedEdgeImg.nj();
    
    //rings radius
    int rads[] = {para.patchSize, para.patchSize};
    int rads_size = 2;
    int patch_size = rads[rads_size-1]*2 + 1; // ? fixed size?
    
    int *data = new int[expanedEdgeImg.nj() * expanedEdgeImg.ni()];
    for (int j = 0; j<expanedEdgeImg.nj(); j++) {
        for (int i = 0; i<expanedEdgeImg.ni(); i++) {
            if (expanedEdgeImg(i, j) == 255) {   // column dominant as in matlab
                data[i * height + j] = 1;
            }
            else{
                data[i * height + j] = 0;
            }
        }
    }
    
    //v_step for faster indexing
    int * v_steps = (int*)malloc(sizeof(int)*patch_size);
    for(int i = 0; i < patch_size; i++){
        v_steps[i] = i*patch_size;
    }
    
    //mask with distances
    int * dist = (int*)malloc(sizeof(int)*patch_size*patch_size);
    for(int i=0; i<patch_size; i++){
        for(int j=0; j<patch_size; j++){
            dist[v_steps[i] + j] = max(abs(i-rads[rads_size-1]), abs(j-rads[rads_size-1]));
        }
    }
    
    
    std::list<line_param> lines;
    
    //normalize values
    float w_c = (width - 1)/2.f;
    float h_c = (height - 1)/2.f;
    float norm = (max(w_c, h_c) - rads[rads_size-1]);
    
    float vec_data[4];
    //for each edge point in image with padds
    for(int i=rads[rads_size-1]; i <  width - rads[rads_size-1]; i++){
        for(int j=rads[rads_size-1]; j <  height - rads[rads_size-1]; j++)
        {
            if (data[i * height + j] == 1)
            {
                //call flood fill with array of rings radius
                std::list<coords> results = floodFill(data, i, j, height, rads, rads_size, v_steps, dist);
                
                //fit ellipse
                if(fit_ellipse(results, vec_data) == 1)
                {
                    //push if ok
                    float c = -vec_data[0]*((j - h_c)/norm) - vec_data[1]*((i - w_c)/norm);
                    lines.push_back(line_param(vec_data[1], vec_data[0], c, 1.f));
                }
            }
        }
    }
    
    // output lines
    for (std::list<line_param>::iterator ite = lines.begin(); ite != lines.end(); ite++) {
        vnl_vector_fixed<double, 4> line;
        line[0] = ite->a;
        line[1] = ite->b;
        line[2] = ite->c;
        line[3] = ite->w;
        outLines.push_back(line);
    }
    
    delete []data;
    free(v_steps);
    free(dist);
    
    printf("find %lu lines\n", outLines.size());
}

std::list<coords> floodFill(int *data, int cx, int cy, int img_height, int * rads, int rads_size, int * v_steps, int * dist)
{
    std::list<coords> points;
    std::list<coords> results;
    
    int patch_size = rads[rads_size-1]*2 + 1;
    int radius = rads[rads_size-1];
    points.push_back(coords(radius, radius, 0));
    
    int * labels = (int*)calloc(patch_size*patch_size, sizeof(int));
    int * hist = (int*)calloc(radius+1, sizeof(int));
    hist[0] = 1;
    
    int eque_min = 0;
    
    coords C(radius, radius, 0);
    
    int x_offset = cx - radius;
    int y_offset = cy - radius;
    
    for(int i=0; i<rads_size; i++)
    {
        while (points.size()!=0 && eque_min <= rads[i])
        {
            for (std::list<coords>::iterator j = points.begin(); j!=points.end(); ++j)
            {
                if(j->d == eque_min)
                {
                    C = (*j);
                    points.erase(j);
                    break;
                }
            }
            
            hist[C.d]--;
            
            if(labels[v_steps[C.x] + C.y] == 0)
            {
                labels[v_steps[C.x] + C.y] = i+1;
                results.push_back(coords(C.y - radius, C.x - radius, i+1));
                
                for(int j=0; j<8; j++)
                {
                    int x = C.x + xinc[j];
                    int y = C.y + yinc[j];
                    if(x >= 0 && x < patch_size && y >=0 && y < patch_size && data[(x + x_offset)*img_height + y + y_offset] == 1 && labels[v_steps[x] + y] == 0)
                    {
                        int d = dist[v_steps[x] + y];
                        points.push_back(coords(x, y, d));
                        hist[d]++;
                    }
                }
            }
            
            for(int j=0; j<radius + 1; j++)
            {
                if(hist[j]!=0)
                {
                    eque_min = j;
                    break;
                }
            }
        }
    }
    
    free(hist);
    free(labels);
    return results;
}

int fit_ellipse(std::list<coords> & results, float * vec_data)
{
    int good_ellipse = 0;
    
    int dist = results.back().d;
    
    while (good_ellipse == 0 && dist > 0)
    {
        float cov_data[3];
        
        //get mean
        float x_mean = 0;
        float y_mean = 0;
        
        int size = 0;
        for (std::list<coords>::iterator it = results.begin(); it!=results.end(); ++it)
        {
            if(it->d > dist) break;
            x_mean += it->x;
            y_mean += it->y;
            size++;
        }
        
        if(size < 3) return good_ellipse;
        
        x_mean /= size;
        y_mean /= size;
        
        //get covariance
        float xx = 0;
        float yy = 0;
        float xy = 0;
        
        for (std::list<coords>::iterator it = results.begin(); it!=results.end(); ++it)
        {
            if(it->d > dist) break;
            float x = it->x - x_mean;
            float y = it->y - y_mean;
            xx += x*x;
            yy += y*y;
            xy += x*y;
        }
        
        cov_data[0] = xx / (size - 1);
        cov_data[1] = xy / (size - 1);
        cov_data[2] = yy / (size - 1);
        
        good_ellipse = eigens(cov_data, vec_data);
        dist--;
    }
    
    return good_ellipse;
}

int eigens(float * cov_data, float * vec_data)
{
    float trace = cov_data[0] + cov_data[2];
    float det = cov_data[0]*cov_data[2] - cov_data[1]*cov_data[1];
    
    float s = sqrt(trace*trace/4 - det);
    float vals_data[2] = {trace/2 - s, trace/2 + s};
    
    if(abs(cov_data[1]) > 0.001 )
    {
        vec_data[0] = vals_data[0] - cov_data[2];
        vec_data[1] = cov_data[1];
        
        float n = sqrt(vec_data[0]*vec_data[0] + vec_data[1]*vec_data[1]);
        vec_data[0] /= n;
        vec_data[1] /= n;
    }else
    {
        vec_data[0] = int(cov_data[0] < cov_data[2]);
        vec_data[1] = vec_data[0] - 1;
    }
    
    vec_data[2] = vec_data[1];
    vec_data[3] = -vec_data[0];
    
    return (abs(vals_data[0]) < 0.001 || vals_data[1]/vals_data[0] > ELLIPSE_THRESH);
}

